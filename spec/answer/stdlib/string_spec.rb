...
RSpec.describe String do
  ...
  describe "#split" do
    ...
    context "引数を渡さなかった場合" do
      ...
      context "対象の文字列に前後に空白文字がある場合" do
        let(:string) { "  homu mami mado    " }

        it { is_expected.to eq ["homu", "mami", "mado"] }
      end

      context "対象の文字列が空の文字列" do
        let(:string) { "" }
    ...
    context "第一引数を渡した場合" do
      ...
      context "空文字列を渡した場合" do
        ...
        it { is_expected.to eq ["h", "o", "m", "u"] }
      end

      context "空の正規表現を渡した場合" do
        let(:string) { "homu" }
        let(:args) { [//] }
    ...
    context "第二引数を渡した場合" do
      ...
      context "0 を渡した場合" do
        ...
        it { is_expected.to eq ["homu", "mami", "mado", "saya", "an"] }
      end
  ...
  describe "#<<" do
    ...
    context "`a` を渡した場合" do
      ...
      it { is_expected.to eq "homua" }
      # string << obj した結果、string の値がどう変わるかを検証するテスト
      # from に変更前の値を渡し、to に変更後の値を渡す
      it { expect { subject }.to change { string }.from("homu").to "homua" }
    end
    ...
    context "`homu` を渡した場合" do
      ...
      it { expect { subject }.to change { string }.from("homu").to "homuhomu" }
    end
  ...
  describe "#contact" do
    ...
    context "引数が複数の場合" do
      let(:args) { ["mami", "mado"] }   # string.concat("mami", "mado") と同等

      it { is_expected.to eq "homumamimado" }
      it { expect { subject }.to change { string }.from("homu").to "homumamimado" }
    end
