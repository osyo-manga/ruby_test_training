...
RSpec.describe Array do
  ...
  describe "#reverse" do
    ...
    it { is_expected.to eq [1, "mami", "homu"] }
  ...
  describe "#shuffle" do
    ...
    context "引数を渡さなかった場合" do
      context "乱数を固定していない場合" do
        let(:kwargs) { {} }   # array.shuffle() と同等

        # 配列の順番は気にせずに配列の要素があっているかどうかをテストする
        it { is_expected.to match_array [1, 2, 3, 4, 5, 6] }
      end

      context "乱数を固定している場合" do
        let(:kwargs) { {} }   # array.shuffle() と同等
        ...
        it { is_expected.to eq [3, 6, 1, 4, 5, 2] }
      end
    end

    context "random: 引数を渡した場合" do
      let(:kwargs) { { random: random } }   # array.shuffle(random: random) と同等
      let(:random) { Random.new(5) }

      it { is_expected.to eq [6, 3, 5, 2, 1, 4] }
    end
  end
  ...
  describe "#sample" do
    ...
    context "3つの要素を受け取る場合" do
      # array.sample(3) と同等
      let(:args) { [3] }
      let(:kwargs) { {} }

      it { is_expected.to eq [2, 6, 5] }
    end
  ...
  describe "#grep" do
    ...
    context "先頭の文字が m の文字列のみを絞り込む" do
  ...
  describe "#grep_v" do
    ...
    context "先頭の文字が m でない文字列を絞り込む" do
      let(:array) { ["homu", "mami", "mado", "an"] }
      let(:pattern) { /^m/ }

      it { is_expected.to eq ["homu", "an"] }
    end

    context "https でない ULR を絞り込む" do
      let(:array) { ["https://docs.ruby-lang.org", "http://example.com", "https://www.ruby-lang.org"] }
      let(:pattern) { /^https/ }
